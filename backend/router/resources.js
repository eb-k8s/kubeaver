...
router.get('/resources/offline', async (ctx, next) => {
  //curl -X GET -H "Content-Type: application/json" http://10.1.70.162:8000/api/resources/offline
  try {
    const data = await serviceResources.getOfflineContents();
    ctx.body = data;
  } catch (error) {
    ctx.body = {
      code: 50000,
