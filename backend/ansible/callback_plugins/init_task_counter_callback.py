...
class CallbackModule(CallbackBase):
    ...
    CALLBACK_NAME = 'init_task_counter_callback'

    def __init__(self):
        super().__init__()
        self.task_count = 0  # 初始化任务计数器
        self.time_list = {"reset": 0.0, "pre_process": 0.0, "download": 0.0,"install k8s cluster": 0.0, "install network_plugin": 0.0, "after_check": 0.0}
        self.stage_task_list = {"reset": 32, "pre_process": 75, "download": 893, "install k8s cluster": 384, "install network_plugin": 106, "after_check":441}
    ...
    def task_play_stage(self, play_name):
        if play_name == "force delete node" or play_name == "Confirm node removal":                                 
            return "reset"
        elif play_name == "Prepare for etcd install":
            return "download"
        elif play_name == "Install Kubernetes nodes" or play_name == "Install the control plane" or play_name == "Install etcd" or play_name == "Install etcd certs on nodes if required" or play_name == "Add worker nodes to the etcd play if needed":
            return "install k8s cluster"
        elif play_name == "Invoke kubeadm and install a CNI" or play_name == "Install Calico Route Reflector" :
            return "install network_plugin"
        elif play_name == "Patch Kubernetes for Windows" or play_name == "Install Kubernetes apps" or play_name == "Apply resolv.conf changes now that cluster DNS is up":
            return "after_check"
        else:
            return "pre_process"
    ...
    def v2_runner_on_ok(self, result):
        ...
        dict4 = {"name": "install k8s cluster", "time": self.time_list["install k8s cluster"]}
